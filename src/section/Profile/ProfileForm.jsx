...
import UserOne from "../../images/user/user-icon-on-transparent-background-free-png.webp";
...
// Компонент для профілю
export default function ProfileForm() {
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.user);
  const { t } = useTranslation();
  const [avatarUrl, setAvatarUrl] = useState(
    user?.avatar ? user.avatar : UserOne
  );
  const [imageFile, setImageFile] = useState(null);

  // Визначення схеми валідації з перекладом
  ...
  // Обробка зміни зображення аватара
  const handleImageChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        // Оновлення аватара в Redux
        dispatch(UpdateAvatar({ avatar: publicUrl }));
      } catch (error) {
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark md:max-w-150">
        <form onSubmit={handleSubmit(onSubmit)}>
          <div className="flex flex-col gap-5.5 p-6.5">
            ...
            {/* Вибір мови */}
            <div>
              <p className="mb-2">{t("Select language:")}</p>
              <LanguageSelector />
            </div>
